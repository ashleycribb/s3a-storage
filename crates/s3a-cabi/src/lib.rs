...
use s3a_engine::{
    Compactor, MmapReader, QuerySieve, S3ACoordinate, S3ACrudEngine, TelemetryRecord,
    TileFusionEngine,
};
...
mod tests {
    ...
    fn test_cabi_open_query_compact_and_fuse() {
        ...
        writer1
            .write_hyper_tile(
                TileType::TELEMETRY,
                &[TelemetryRecord::new(500, 1, 10, 99.9)],
                Some(&[500]),
                None,
            )
            .unwrap();
        ...
        writer2
            .write_hyper_tile(
                TileType::TELEMETRY,
                &[TelemetryRecord::new(600, 1, 10, 101.2)],
                Some(&[600]),
                None,
            )
            .unwrap();
        ...
        unsafe {
            ...
            let mut out_len = 0;
            let res =
                s3a_query_telemetry(handle, 0, 1000, out_records.as_mut_ptr(), 10, &mut out_len);
            assert_eq!(res, 0);
            assert_eq!(out_len, 2);
